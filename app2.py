...
with st.sidebar:
    # Dropdown to select professor's webpage
    selected_professor = st.selectbox("Select professor:", found_professors)
    if selected_professor:
        professor_data = data[selected_professor]
        st.write(f"**Name:** {selected_professor}")
        st.write(f"**Research Interest:** {professor_data['Research']}")
        # st.write(f"**Publications:** {professor_data['Publications']}")
        st.write(f"**Email:** {professor_data['Email']}")
        if professor_data['ProfilePic']:
        ...
        # Generate a unique key using professor's name
        button_key = f"button_{selected_professor}_dropdown"
